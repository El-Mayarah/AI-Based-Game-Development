...
void ASpawnVolume::SpawnTheActor_Implementation(UClass* ToSpawn, const FVector& Location) {
	if (ToSpawn) {
		...
		if (World) {
			...
			if (Hostile) {
				Hostile->SpawnDefaultController();

				AAIController* AICont = Cast<AAIController>(Hostile->GetController());
				if (AICont) {
					Hostile->AIController = AICont;
				}
			}
		}
	}
